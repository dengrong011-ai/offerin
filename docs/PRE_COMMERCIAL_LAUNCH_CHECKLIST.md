...
- **现状**：统一白名单在 `api/_cors.ts`（`ALLOWED_ORIGINS`），被 `api/gemini/proxy.ts`、`api/xorpay/create.ts`、`api/xorpay/query.ts` 使用；含 `https://offerin.co`、`https://www.offerin.co` 及本地开发 origin。
- **确认**：若正式域不同（如多域名、子域），只需在 **`api/_cors.ts`** 中修改 `ALLOWED_ORIGINS` 并部署即可。
...
| [ ] CORS 含正式域名 | 与 `api/_cors.ts` 中 `ALLOWED_ORIGINS` 一致 |
