/**
 * XorPay 支付回调接口 (Vercel Serverless Function)
 * 
 * 当用户支付成功后，XorPay 会向这个接口发送 POST 请求
 * 回调地址格式：https://你的域名/api/xorpay/notify
 */
 ...
 * 生成签名
 */
function generateSign(...params: string[]): string {
 ...
 * 验证 XorPay 回调签名
 */
function verifySign(aoid: string, orderId: string, payPrice: string, payTime: string, sign: string): boolean {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('收到 XorPay 回调:', { aoid, order_id, pay_price, pay_time });
    ...
    console.log('支付回调处理成功:', order_id);
    
    // 返回 success 告诉 XorPay 已收到通知
    return res.status(200).send('success');
